...
#* Distro
dtr_name="distro_name";  #* Distro name (distro_name)

#* Image
img_name="image_name";  #* Image name (project_img)
img_ver="latest";  #* Image version (latest)

#* Container
ctn_name="container_name";  #* Container name (project_ctn)

#* Publish To
host_ip="127.0.0.1";  #* Host IP (127.0.0.1 or localhost)
host_port="80";  #* Docker host port (80)       http://localhost:80
ctn_port="8080";  #* Container port (8080)      Exposed 8080 (comment)

#* Volume(s) binding
vol_src+=("/source");  #* Local/Windows directory (/folder_in_project_directory)
vol_trg+=("/target");  #* Container directory (/path/to/container/folder)
...
cmdShell () {
    local cmd="docker exec -it $ctn_name /bin/bash";  #* Open Container Shell
